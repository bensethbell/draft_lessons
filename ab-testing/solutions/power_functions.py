...
class ProportionPower(object):
    ...
    def calc_power(self):
        ...
        z_beta = (sqrt(self.total) -
                  self.z_alpha * a_sqrt / self.p_delta) * self.p_delta / b_sqrt
...
if __name__ == '__main__':
    ...
    test_alpha = .05
    test_power = .8
    pp_obj = ProportionPower(test_p_samc, test_p_samt, test_effect_size,
                             alpha=test_alpha, power=test_power)
    test_total = pp_obj.calc_min_sample()  # 1396
    print 'total', test_total
    pp_obj.total = test_total
